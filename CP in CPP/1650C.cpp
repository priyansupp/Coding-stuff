...
void solve(){
    ...
    //  The sum of weights of the first 2⋅n points will be minimal, so we use them to construct a system of n nested segments. 
    ll sum = 0;
    forr(i, 0, 2 * n){
        sum += a[i].weight;
    }
    ...
    // just printing indices of pairs of x-coordinate that form nested interval.
    for(int i = 0; i < n; i++){
        cout << a[i].ind << " " << a[2 * n - 1- i].ind << '\n';
    }
