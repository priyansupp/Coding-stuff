...
void solve(){
    input(n, a);
    srt(a);
    vector<int>v;       // v stores frequency of each type of powerup
    for(int i = 0; i < n;){
        int freq = 1;
        int j = i + 1;
        if(j < n)
    ...
    // v = {2,3,2,1,2};
    // k = 1 : {1,1,2,2,2,4,4,5,6,6};
    // k = 2 : {1,1,2,2,2,4,4,6,6}, {5};
    // k = 3 : {1,1,2,2,2,4,4} , {5} , {6, 6};
    // k = 4 : {1,1,2,2,2} , {5} , {6, 6}, {4, 4};
    // k = 5 : {1,1} , {5} , {6, 6}, {4, 4}, {2,2,2};
    // hence till k = 5, output will be 5 only(number of distinct elements i.e v.size())
    // then all subgroups would begin to expand and increase the length by 1 until the total length become n
    // k = 5 : {1}, {1} , {5} , {6, 6}, {4, 4}, {2,2,2};
