...
int arr[8] = {11, 111, 1111, 11111, 111111, 1111111, 11111111, 111111111};

bool possible(int i, int sum, vector<vector<int>> &dp){
    if(i == 0){
        if(sum % arr[0] == 0){
            return true;
        }
    ...
    bool notpick = possible(i - 1, sum, dp);
    bool pick = false;
    if(sum >= arr[i]) pick = possible(i, sum - arr[i], dp);
