...
void solve(){
    int l, r, a;
    cin >> l >> r >> a;
    // we've to maximize quotient and remainder...
    int ans = (r / a) + (r % a);    // r gives maximal possible quotient max(x / a) when x = r but r % a may not be maximal remainder so we test for another case by sacrificing on quotient by 1 and making remainder (a -  1)
    int m1 = ((r / a) * a) - 1;     // gives quotient ((r / a) - 1) and remainder (a - 1)
    if(m1 >= l) ans = max(ans, (m1 / a) + m1 % a);
    cout << ans << '\n';
    return;
}
