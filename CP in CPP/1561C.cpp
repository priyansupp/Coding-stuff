...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        ...
        for(int j = 1; j <= k; j++) {
            cin >> a[i][j];
            maxi[i] = max(maxi[i], a[i][j] + 1 - (j - 1));   // calculating minimum power the hero must in that cave.
        }
        v.pb({maxi[i], maxi[i] + k});                        // pair of powers he enter and exits with
    }
    srt(v);
    for(int i = n - 2; i >= 0; i--){
        if(v[i].second < v[i + 1].first){
            v[i].first += v[i + 1].first - v[i].second;
        }
    }
    cout << v[0].first << '\n';
    return;
