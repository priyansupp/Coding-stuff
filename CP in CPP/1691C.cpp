...
void solve(){
    ...
    int dist1 = pos.front(), dist2 = n - 1 - pos.back();
    if(pos.size() == 1){
        if(dist2 <= k) cout << 1 << '\n';
        else if(dist1 <= k) cout << 10 << '\n';
        else cout << "11\n";
        return;
    }
    if(dist1 == 0){
        if(dist2 == 0 || dist2 <= k) cout << 11 * pos.size() - 11 << '\n';
        else cout << 11 * pos.size() - 1 << '\n';
        return;
    }
    if(dist2 == 0){
        if(dist1 == 0 || dist1 <= k) cout << 11 * pos.size() - 11 << '\n';
        else cout << 11 * pos.size() - 10 << '\n';
        return;
    }
    if(dist1 + dist2 <= k){
        cout << 11 * pos.size() - 11 << '\n';
