...
ll maxvalue(vector<pair<int, int>> &item, int i, int n, int w, vector<vector<ll>> &dp){
    if(i == n){
        return 0;
    }
    if(dp[i][w] != -1) return dp[i][w];
    ll notpick = 0 + maxvalue(item, i + 1, n, w, dp);
    ll pick = 0;
    if(w >= item[i].first) pick = item[i].second + maxvalue(item, i + 1, n, w - item[i].first, dp);
...
void solve(){
    ...
    forr(i, 0, N) cin >> item[i].first >> item[i].second;
    vector<vector<ll>>dp(N, vector<ll>(W + 1, -1));
    cout << maxvalue(item, 0, N, W, dp);
    return;
}
