...
//--------------SOLVE--------------//
void solve(){   // check only the 0th bit of y. summation and addition at 0th bit are same, so all numbers would give the same res(regardless of operation performed) on the 0th bit. The only difference will occur while choosing x(odd/even) or x+3(even/odd)
    ll n, x, y;
    cin >> n >> x >> y;
    // vector<ll>a(n);
    int res = 0;
